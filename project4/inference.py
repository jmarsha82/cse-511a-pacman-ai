    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for legal in self.legalPositions:
                ...
                self.beliefs[legal] *= emissionModel[distance]
            self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            tmpParticles = util.Counter()
            for part in self.particles:
                distance = util.manhattanDistance(part, pacmanPosition)
                tmpParticles[part] += emissionModel[distance]
            self.beliefs = tmpParticles
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for i in self.particles:
            temp = self.getPositionDistribution(
                self.setGhostPosition(gameState, i))
            newDistribution.append(util.sample(temp))
        self.particles = newDistribution
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefsDistribution = util.Counter()
        for part in self.particles:
            beliefsDistribution[part] += 1.0
        beliefsDistribution.normalize()
        return beliefsDistribution
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ghostPosSet = setGhostPositions(gameState, prevGhostPositions)
                newTemp = self.ghostAgents[i]
                newPosDist = getPositionDistributionForGhost(
                    ghostPosSet, i, newTemp)
                if active[i+1]:
                    parListTemp.append(util.sample(newPosDist, None))
                else:
                    parListTemp.append(inactive[i])
            parList.append(tuple(parListTemp))
            parListTemp = list()
        newParticles = parList
    ...
    def observeState(self, gameState):
        ...
        for oldPs in self.particles:
            ...
            newPs.append(newParticle)

        for newParticle in newPs:
            weight = 1
            for i in rangeNumGhost:
                if noisyDistances[i] != None:
                    temp = util.manhattanDistance(newParticle[i], pacmanPosition)
                    weight *= emissionModels[i][temp]
